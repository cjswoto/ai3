...
def voice_control(update_status_lambda, window, on_submit_callback, prompt_entry, status_label, logo_label, logo_image, voice_options, model_options, voice_var, model_var, voice_menu):
    ...
    global mode
    mode = "monitor"
    logging.debug(f"In Voice_Command")
    with sr.Microphone() as source:
        logging.info("Microphone is now active.")
        while True:
            try:
                logging.debug(f"In Voice_Command" + mode)
...
def main():
    ...
    status_label = tk.Label(window, text="Currently monitoring. Say 'AI3' to wake up or 'ChatGPT' to open the chat.", fg="blue")
    status_label.pack()

    # Configure voice and model selection
    voice_options = list_voices()
    voice_var = StringVar(window)
    voice_var.set(voice_options[1])  # Default or previously saved voice
    ...
    model_var.set(model_options[1])  # Default or previously saved model
