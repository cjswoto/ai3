...
def on_submit(model_var, voice_var, prompt_entry, response_display, status_label):
...
def main():
    ...
    listening_thread = threading.Thread(target=lambda: voice_control(lambda msg: update_status_message(msg, status_label), window, lambda: on_submit(model_var, voice_var, prompt_entry, response_display, status_label), prompt_entry), daemon=True)
